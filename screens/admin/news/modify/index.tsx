...
const NewsModifyScreen = ({ data }: IProps) => {
    ...
    const uploadImage = async () : Promise<boolean> => {
        const exts = news.images.filter(img => img.file).map(item => {
        ...
        <NewsContainer>
            ...
            <div className="n-field">
                ...
                <label className="image-button" htmlFor="image-button">
                    <input style={{ display: "none" }} ref={imageRef} onChange={onChangeImage} multiple type="file" accept="image/*" id="image-button" />
                    <span>이미지 등록</span>
                </label>
                {news.images.length ? <PicSortComponent
                    images={news.images}
                    onSortEnd={onSortEnd}
